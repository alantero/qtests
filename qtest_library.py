...
def qtest_chi2FromFit(x, y, ref, amp, n_std=3, cut_val=2, is_image=False):
    ...
    return {'name':'QTChi2FromFit','result':Result, 'chi2':y, 'run_ref':None, 'amp':amp}
...
def qtest_chi2(x, y, ref, amp, n_std = 3, cut_val = 2, is_image = False):
    ...
    if is_image:
        ...
        if type(y) is list:
            ...
            return {'name':'QTChi2','result':np.nan, 'chi2_ndf':np.nan, 'run_ref':ref['run'], 'amp':amp} 
    else:
    ...
    return {'name':'QTChi2','result':Result, 'chi2_ndf':chi2_ndf, 'run_ref':ref['run'], 'amp':amp}
...
def qtest_ks(x, y, ref, amp, n_std = 3, cut_val = 5e-3, is_image = False):
    ...
    if is_image:
        ...
        if type(y) is list:
            ...
            ### Dont apply to image ME
            return {'name':'QTKstest','result':np.nan, 'p_value':np.nan, 'run_ref':ref['run'], 'amp':amp} 
    else:
    ...
    return {'name':'QTKstest','result':Result, 'p_value':p_value, 'run_ref':ref['run'], 'amp':amp}
...
def qtest_tt(x, y, ref, amp, n_std = 3, cut_val = 5e-3, is_image = False):
    ...
    if is_image:
        ...
        if type(y) is list:
            ...
            ### Dont apply to image ME
            return {'name':'QTTtest','result':np.nan, 'p_value':np.nan, 'run_ref':ref['run'], 'amp':amp} 
    else:
    ...
    return {'name':'QTTtest','result':Result, 'p_value':p_value, 'run_ref':ref['run'], 'amp':amp}
...
def qtest_residuals_per_image(x ,y, ref, amp, n_std = 3, cut_val = 5, is_image = False):
    ...
    ## Calculates mean Standard deviation of reference
    ...
    return {'name':'QTresiduals','result':Result, 'x':x_fail, 'y':y_fail, 'mean_ref':mean_ref,'n_std':n_std, 'std_ref':std_ref, 'run_ref':ref['run'], 'amp':amp}
...
def qtest_residuals_per_rowCol(x, y, ref, amp, n_std = 3, cut_val = 5, is_image = False):
    ...
    if DISPLAY_PLOTS:
        ...
        plt.show()
    if DISPLAY:
        print({'Result':Result[0], 'x_fail':x_fail, 'y_fail':y_fail, 'mean_ref':mean_ref, 'n_std':n_std, 'std_ref':std_ref})
